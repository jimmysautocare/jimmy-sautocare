...
html_original = """            <div class="service-card fade-up">
...
import re
pattern = re.compile(r'(\s*)<div class="(service-card[^"]*)">(.*?)<h3>(.*?)</h3>(.*?)</div>', re.DOTALL)

def replacer(match):
    indent = match.group(1)
    classes = match.group(2)
    before_h3 = match.group(3)
    title = match.group(4).strip()
    after_h3 = match.group(5)
    
    back_items = details.get(title, ["Expert Service", "Quality Parts", "Guaranteed Workmanship", "Transparent Pricing"])
    lis = "\\n".join([f"{indent}                    <li>{item}</li>" for item in back_items])
    
    front = f"""{indent}    <div class="service-card-inner">
...
{indent}    </div>"""
    ...
    return f'{indent}<div class="{classes}" onclick="this.classList.toggle(\\\'flipped\\\')">\n{front}\n{indent}</div>'

new_html = pattern.sub(replacer, html_original)

print(new_html)
with open("c:\\Users\\jenis\\Downloads\\MANDA\\scratch\\repl.txt", "w", encoding="utf-8") as f:
    f.write(new_html)
